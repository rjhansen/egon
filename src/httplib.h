...
private:
    ...
    using HandlersForContentReader = std::vector<std::pair<std::unique_ptr<detail::MatcherBase>,
...
namespace detail {
    ...
    namespace fields {
        ...
        {
            return std::isalnum(c) || c == '!' || c == '#' || c == '$' || c == '%' || c == '&' || c == '\'' || c == '*' || c == '+' || c == '-' || c == '.' || c == '^' || c == '_' || c == '`' || c == '|' || c == '~';
        }
...
inline std::string encode_uri(const std::string& value)
{
    ...
    for (auto c : value) {
        if (std::isalnum(static_cast<uint8_t>(c)) || c == '-' || c == '_' || c == '.' || c == '!' || c == '~' || c == '*' || c == '\'' || c == '(' || c == ')' || c == ';' || c == '/' || c == '?' || c == ':' || c == '@' || c == '&' || c == '=' || c == '+' || c == '$' || c == ',' || c == '#') {
            escaped << c;
        } else {
            escaped << std::uppercase;
            escaped << '%' << std::setw(2)
    ...
    bool need_apply_ranges)
{
    ...
    if (close_connection || req.get_header_value("Connection") == "close" || 400 <= res.status) { // Don't leave connections open after errors
    ...
    const std::vector<std::string>& trusted_proxies)
{
    ...
    for (size_t i = 0; i < ip_list.size(); ++i) {
        ...
        auto is_trusted_proxy = std::any_of(trusted_proxies.begin(), trusted_proxies.end(),
